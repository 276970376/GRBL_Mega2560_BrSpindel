...
// Begin BlueOrangeLive Test with ESC-Brushless-Spindel-Motor
  #ifdef BRUSHLESS_SPINDEL
    SPINDLE_OCR_REGISTER = OUT_BRSP_Steht; // Set PWM pin output to 0,8 ms
    SPINDLE_TCCRA_REGISTER |= (1<<SPINDLE_COMB_BIT); // ESC needs PWM-Sigal  - Ensure PWM output is enabled.
    SPINDLE_ENABLE_DDR |= (1<<SPINDLE_ENABLE_BIT); // Configure as output pin.
    SPINDLE_DIRECTION_DDR |= (1<<SPINDLE_DIRECTION_BIT); // Configure as output pin.
  #else
// End BlueOrangeLive Test with ESC-Brushless-Spindel-Motor
    SPINDLE_OCRA_REGISTER = SPINDLE_OCRA_TOP_VALUE; // Set the top value for 16-bit fast PWM mode
    SPINDLE_ENABLE_DDR |= (1<<SPINDLE_ENABLE_BIT); // Configure as output pin.
    SPINDLE_DIRECTION_DDR |= (1<<SPINDLE_DIRECTION_BIT); // Configure as output pin.
  #endif // BlueOrangeLive +
        ...
        if (rpm > IN_BRSP_START)
        {
         ...
         else
          {
            if (rpm > 1) {current_pwm = OUT_BRSP_START;}
              ...
              {current_pwm = OUT_BRSP_Steht;}  // ungefähr 0,8 milliseconds
           }
       #else
    //  Ende  BlueOrangeLive  Test with ESC-Brushless-Spindel-Motor
